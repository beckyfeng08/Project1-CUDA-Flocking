  ...
  void mainLoop() {
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      if (time - timebase > 1.0) {
        ...
        frame = 0 ;
#if COUNTING_FRAMES
        if (time > 5.0 && time < 15.0) { // discard first 5 seconds
            count++;
            ave_fps += (fps - ave_fps) / count;
            std::cout << " average fps is: " << ave_fps << std::endl;

        }
        else if (time > 15.0) {
            std::cout << "FINAL average fps is: " << ave_fps << std::endl;
